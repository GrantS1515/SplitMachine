...
export const newStateShiftFn = id => errMsg => args => sps => pipe(sps, Sp.leadRight(args.strLen), E.map(args.testFn), E.chain(b => B.match(() => E.left(newErr(errMsg)), () => Sp.shiftLeft(args.strLen)(sps))(b)), E.map(sp => ({ name: "State", id: id, split: sp })));
